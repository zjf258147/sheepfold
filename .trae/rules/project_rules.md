...
| 条目 | 内容 |
|------|------|
| 规则 | 当用户说"执行完整检测清单"时，AI 必须按 `检测项目清单.md` 中定义的 7 步顺序，逐一执行全部检查（含 APK 打包检查）。 |
| 触发词 | **"执行完整检测清单"** |
...
| ⑤ | APK 打包检查 | 运行 `scripts/build_apk.ps1`（或手动逐项验证：git status + 关键文件 + 构建 + 产物） |
| ⑥ | 上线检查 | 数据库迁移、种子数据、环境配置、浏览器兼容性（人工项） |
| ⑦ | 数据字典更新 | `cd backend && uv run python scripts/generate_data_dict.py` |
