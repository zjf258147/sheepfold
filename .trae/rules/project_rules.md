...
| 条目 | 内容 |
|------|------|
| 规则 | 当用户说"执行完整检测清单"时，AI 必须按 `检测项目清单.md` 中定义的 7 步顺序，逐一执行全部检查（共覆盖清单 v2.11 全部 29 项未完成项，含 1 项开发项）。 |
| 触发词 | **"执行完整检测清单"** |
...
| 步骤 | 内容 | 覆盖清单章节 | 命令/操作 |
|:---:|------|:---:|-----------|
| ① | **后端测试** | §1.1~1.3, §1.5, §1.6 | `pytest tests/ -v --tb=short`（已有测试）；**额外**：创建并跑 `tests/smoke/` API 全量冒烟（94端点）；创建并跑序列化异常测试 §1.3.10 |
| ② | **前端测试** | §1.4 | `npm run test`（Vitest 单元测试）；**额外**：检查二期 5 个测试文件是否存在（warranty/stocktake/deviceLedger/polling/appConfig），若缺失则先创建再跑 |
| ③ | **Skill 代码质量检查** | §2.1~2.4, §3.1~3.6 | `code-quality-skill` → `detect-code-smells` → `code-review-and-quality`；**额外**：密钥管理检查（.env/.env.example/前端硬编码/日志泄漏）、全局异常处理器检查、参考实现复用检查 |
| ④ | **技术栈专项检查** | §2.2 | 按修改范围调用：FastAPI / Vue / SQLAlchemy+Alembic / Pinia / MySQL / Docker |
| ⑤ | **专项测试补充** | §4.1~4.19 | **前端路由遍历**（23页面500检查）→ **多角色遍历** → **打印功能**（6种单据+分页）→ **跨浏览器渲染**（Chrome/Edge/Firefox/Safari §4.7.1-3）→ **浏览器打印兼容**（§4.7.4-6）→ **Excel 5000行**导出 → **50用户并发** → **慢查询EXPLAIN** → **大数据量渲染** → **404路由/请求超时** → **UI视觉一致性** → **Git规范检查**（commit格式+里程碑Tag）→ **E2E测试**（13用例，需前端服务）→ **轮询提醒开发验证**（§4.10，先实现功能再测） |
| ⑥ | **APK 打包检查** | §4.17, §4.14 | `scripts/build_apk.ps1`（git status + 关键文件 + 构建 + 产物验证，共22项） |
| ⑦ | **上线检查 + 数据字典** | §5.1~5.5, §6.1 | 数据库迁移备份恢复 → 环境配置 → 二期功能验证 → 性能验证（系统启动<10s）→ **需求验收对照**（4主线+二期+权限矩阵，对照PRD逐条） → 数据字典更新 `generate_data_dict.py` |
