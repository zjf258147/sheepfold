...
| 条目 | 内容 |
|------|------|
| 规则 | 当用户说"执行完整检测清单"时，AI 必须按 `检测项目清单.md` 中定义的 7 步顺序，逐一执行全部检查（共覆盖清单 v2.11 全部 29 项未完成项）。 |
| 触发词 | **"执行完整检测清单"** |
...
| ② | **前端测试** | §1.4 | `npm run test`（Vitest 单元测试） |
...
| ⑤ | **专项测试补充** | §4.1~4.19 | **前端路由遍历**（23页面500检查）→ **多角色遍历** → **打印功能**（6种单据+分页+浏览器兼容）→ **Excel 5000行**导出 → **50用户并发** → **慢查询EXPLAIN** → **大数据量渲染** → **404路由/请求超时** → **UI视觉一致性** → **Git规范检查**（commit格式+里程碑Tag）→ **E2E测试**（13用例，需前端服务） |
