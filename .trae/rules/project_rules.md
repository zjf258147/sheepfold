...
### 规则24：表单精简优化检测清单（不可协商）

| 条目 | 内容 |
|------|------|
| 规则 | 表单精简优化方案的验证与检测。当用户说"执行完整检测清单"时，在阶段2中追加表单优化验证。当用户说"检测表单优化"、"表单优化验证"、"优化检测清单"时，独立执行此规则。 |
| 详细方案 | `doc/表单精简与效率提升优化方案.md`（第 七 章验证方案 + 第 八 章检测清单） |
| 触发词 | "检测表单优化"、"表单优化验证"、"优化检测清单"、"验证优化项" |
...
用户说 "检测表单优化"
    ↓
步骤1：读取 doc/表单精简与效率提升优化方案.md 第八 章检测清单
    ↓
步骤2：按 8.2→8.3→8.4→...→8.11 顺序逐项检查
    对每项：检查代码是否存在 → 如果存在则执行验证 → 记录结果
    ↓
步骤3：执行回归验证（8.10）
    - 后端 pytest
    - 前端 npm run build
...
| 编号 | 检查项 | 优先级 | 验证方式 |
|:---:|--------|:---:|------|
| O-1 | `datetime.js` 快捷时间配置 | P0 | 代码检查 |
| O-2 | 8个页面 shortcuts 引入 | P0 | 代码检查 |
| O-3 | 6个页面默认时间范围 | P0 | 代码检查 + 页面验证 |
| O-4 | 字段联动自动填充 | P0 | 代码 + 页面验证 |
| O-5 | Shipment 表单精简 | P1 | 代码 + 页面验证 |
| O-6 | Inbound `unit_price` 条件显示 | P1 | 代码 + 页面验证 |
| O-7 | Customer 表单分组折叠 | P2 | 代码 + 页面验证 |
| O-8 | RmaReturn 表单精简 | P2 | 代码 + 页面验证 |
| O-9 | DeviceLedger 折叠 | P2 | 代码 + 页面验证 |
| O-10 | 筛选条件 localStorage 记忆 | P1 | 代码 + 页面验证 |
| O-11 | 表单自动保存草稿 | P1 | 代码 + 页面验证 |
| O-12 | 扫码连续录入 | P1 | 代码 + 页面验证 |
| O-13 | 弹窗自动聚焦 | P2 | 代码 + 页面验证 |
| O-14 | 一键复制创建 | P2 | 代码 + 页面验证 |
