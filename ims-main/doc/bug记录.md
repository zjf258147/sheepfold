...
| 严重程度 | 低 |
| 模块 | 前端 - 登录页 |
| 现象 | Playwright 自动化测试中 `button:has-text("登录")` 选择器无法匹配登录按钮，导致 E2E 登录测试失败 |
| 根因 | `Login.vue` 中登录按钮文本为 `登 录`（含空格），而自动化选择器使用 `登录`（无空格），不匹配 |
| 修复方案 | E2E 测试使用 `button:has-text("登")` 部分匹配 |
| 涉及文件 | `tests/e2e/test_e2e_flows.py`、`tests/browser_tests.py` |
| 教训 | 前端按钮文本中的空格会影响自动化测试选择器；建议前端使用无空格文本或测试使用部分匹配（`has-text("部分")`） |
