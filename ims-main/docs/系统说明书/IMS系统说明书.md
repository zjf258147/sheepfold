...
> **文档版本**：v2.14 | **更新日期**：2026-09-18 | **作者**：文档组
...
| 版本 | 日期 | 变更内容 | 作者 |
|------|------|----------|------|
| v2.14 | 2026-09-18 | API角色限制全面实施(8模块)、数据字典更新(41表/499字段)、E2E验证通过 | 文档组 |
...
本文档适用于 IMS 生产物料与产品追溯管理系统 v2.14。
...
| 返厂诊断 | 创建诊断报告 | `ADMIN` + `QUALITY` |
| 维修操作 | 执行维修 | `ADMIN` + `PRODUCTION` |
| 维修质检 | 创建质检报告 | `ADMIN` + `QUALITY` |
| 入库确认 | 维修入库 | `ADMIN` + `WAREHOUSE` |
| 报废审批 | 审批报废 | `ADMIN` 专属 |
| 出货管理 | 全部 | `ADMIN` + `WAREHOUSE` |
| 出库管理 | 全部 | `ADMIN` + `WAREHOUSE` |
| BOM管理 | 编辑 | `ADMIN` + `PRODUCTION` |
| 库存盘点 | CRUD | `ADMIN` + `WAREHOUSE` |
| 库存调整 | CRUD | `ADMIN` + `WAREHOUSE` |
| 商品SKU | 编辑 | `ADMIN` 专属 |
| 往来单位 | 编辑 | `ADMIN` 专属 |
| 设备台账 | CRUD | `ADMIN` + `WAREHOUSE` |
| 客户管理 | CRUD | `ADMIN` + `WAREHOUSE` |

> 后端API已全面实施角色依赖注入（`AllowWarehouseOrAbove`/`AllowQualityOrAbove`/`get_current_admin`），绕过前端直接调用API将被拦截返回403。
...
> 完整字段定义、类型、约束详见《数据字典》（`docs/附录/数据字典.md`），由 `generate_data_dict.py` 自动生成，当前含 **41 张表、499 个字段**。
> 本文档仅描述业务含义与使用场景。
