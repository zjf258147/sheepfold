...
const allDocs = computed(() => {
  if (props.repairs && props.repairs.length > 0) {
    return props.repairs.map(r => r.data || r)
  }
  if (props.data) {
...
<template>
  <div class="print-document repair-print">
    <template v-for="(doc, idx) in allDocs" :key="doc.repair_no || idx">
    <div class="print-page" :class="{ 'batch-page': idx < allDocs.length - 1 }">
      ...
      <div class="page-footer">
        <span>{{ allDocs.length > 1 ? `第 ${idx + 1} 页 / 共 ${allDocs.length} 页（维修工单）` : '第 1 页 / 共 1 页' }}</span>
      </div>
    </div>
    </template>
  </div>
