...
import { getItemHistory } from '@/api/inventory'
import { listSnapshotDates, listSnapshotItems, exportSnapshotItems } from '@/api/snapshot'
...
import { formatDateInput } from '@/utils/snapshotLedger'
...
const hasDetailScope = computed(() => !!query.value.snapshot_date)
const detailScopeLabel = computed(() => (
  query.value.snapshot_date ? `${query.value.snapshot_date} 日快照` : ''
))
const detailTableKey = computed(() => `${query.value.snapshot_date}-${page.value}`)
const snapshotDateSet = computed(() => new Set(dates.value))

function disableSnapshotDate(time) {
  if (!dates.value.length) return true
  return !snapshotDateSet.value.has(formatDateInput(time))
}
...
async function loadDates() {
  ...
  if (dates.value.length && !query.value.snapshot_date) {
    query.value.snapshot_date = dates.value[0]
  }
}
...
function buildFilterParams() {
  const params = {}
  if (query.value.snapshot_date) params.snapshot_date = query.value.snapshot_date
...
async function loadData() {
  ...
  try {
    ...
    const res = await listSnapshotItems(params)
...
async function handleExport() {
  if (!hasDetailScope.value) {
    ElMessage.warning('请先选择快照日期或月份')
    return
  }
  exportLoading.value = true
  try {
    const blob = await exportSnapshotItems(buildFilterParams())
    const label = query.value.snapshot_date
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `库存快照明细_${label}.xlsx`
...
function resetQuery() {
  query.value = {
    snapshot_date: dates.value.length ? dates.value[0] : '',
    keyword: '',
    category_id: null,
    sku_id: null,
    stock_status: '',
...
<el-form :inline="true" class="list-search" @submit.prevent="handleSearch">
  ...
  <el-form-item>
    <el-button type="primary" native-type="button" @click="handleSearch">查询</el-button>
    <el-button native-type="button" @click="resetQuery">重置</el-button>
    <el-button
      type="success"
      :icon="Download"
      :loading="exportLoading"
      :disabled="!hasDetailScope"
      @click="handleExport"
    >
      导出库存
    </el-button>
...
<p v-if="hasDetailScope" class="hint">当前查看：{{ detailScopeLabel }}，共 {{ total }} 条</p>
<p v-else-if="!dates.length" class="hint warn">暂无日快照数据，请先执行日快照任务或运行演示数据脚本。</p>

<el-table :key="detailTableKey" :data="items" v-loading="loading" stripe empty-text="暂无快照明细">
